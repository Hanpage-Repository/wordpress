	...
	function mbw_get_item_template($mode, $data){
		...
		if(!empty($data["type"])){
			if(!empty($data["combo"])){
				...
				if(!empty($data["combo"]["class"])) $data["combo"]["class"]		= ' class="'.$data["combo"]["class"].'"';
				else $data["combo"]["class"]		= '';
				...
				$event_type		= "onchange";
				if($data["type"]=="checkbox" || $data["type"]=="radio") $event_type		= "onclick";

				$data["ext"]		= $data["ext"]." ".$event_type."=\"template_match_handler('".$data["type"]."',this,'".$combo_data["item_name"]."','".$combo_data["match_type"]."','".$combo_data["match_value"]."')\"";

				$value			= "";
				if(isset($data["value"])) $value		= $data["value"];
				$template_start			.= mbw_create_template($mode, $data);
				$data							= array_merge($data,$combo_data);
				if($mode=="comment"){
				...
				if(isset($value)){					
					if(($combo_data["match_type"]=="show" && $combo_data["match_value"]!=$value) || ($combo_data["match_type"]=="hide" && $combo_data["match_value"]==$value)){
						$style		.= "display:none;";
					}
				}else{
					if($combo_data["match_type"]=="show") $style		.= "display:none;";
				}				
				$template_start	.= '<'.$parent_tag.' id="combo_'.$combo_data["item_name"].'" style="'.$style.'">'.mbw_create_template($mode, $data);
...
if(!function_exists('mbw_create_template')){
	function mbw_create_template($mode, $data){
		...
		if(!empty($data["type"])){
			...
			else if(empty($template_start) && function_exists($templates[$key])) $template_start		= call_user_func("mbw_get_input_template",$mode, $data);			
		}
		return $template_start;
	}
}
