...
if($mode=="html5"){
	...
	if(!empty($headers)){
		...
		if(!empty($upload_data["path"])){
			...
			$sFileInfo .= "&sFileName=".$file_name;
			$sFileInfo .= "&sFileURL=".mbw_get_image_url("path",$upload_data["path"]);
			echo $sFileInfo;
		}else{
	...
	if(!empty($_FILES["Filedata"]) && mbw_check_api_file("editor")) {
		...
		if($mstore->get_result_data("state")=="error"){
			...
			$file_data["url"]			= mbw_get_image_url("path",$upload_data["path"]);
	...
	if((!empty($_FILES["Filedata"]) || !empty($_FILES["upload"]))&& mbw_check_api_file("editor")) { 		
		...
		if(!empty($upload_data["path"])){
			if(!empty($_FILES["upload"]) && !empty($_GET['CKEditorFuncNum'])){
				...
				$url .= "&bNewLine=true";
				$url .= "&sFileName=".rawurlencode(rawurlencode($file_name));
				$url .= "&sFileURL=".mbw_get_image_url("path",$upload_data["path"]);
				header('Location: '. $url);
			}
