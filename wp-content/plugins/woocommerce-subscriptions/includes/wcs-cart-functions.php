...
function wcs_cart_totals_order_total_html( $cart ) {
	...
	$str_temp1 = str_replace("month for", "매 월, ", wp_kses_post( apply_filters( 'wcs_cart_totals_order_total_html', wcs_cart_price_string( $value, $cart ), $cart ) ));
	$str_temp1 = str_replace("months", "개월 동안(첫 달의 비용은 초기 결제에 포함)", $str_temp1);
	$str_temp1 = str_replace("First renewal", "다음 결제일", $str_temp1);
