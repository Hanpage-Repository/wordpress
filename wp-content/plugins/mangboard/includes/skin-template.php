	...
	function mbw_get_list_template($data,$tag=null,$echo=true){
		...
		if(!empty($data["type"])){
			if($data["type"]=='date'){
				...
				$template_start	.= '<input'.$data["ext"].__STYLE($data["style"]).' type="checkbox" name="check_array[]" value="'.mbw_get_board_item('fn_pid').'"/>';

			}else if($data["type"]=='hidden_pid'){
				$template_start	.= '<span>'.$data["value"].'</span><input type="hidden" name="'.$data["item_name"].'_array[]" value="'.$data["value"].'" />';
				...
				if(mbw_get_param("list_type")=="list"){
					if(mbw_get_board_item("fn_category1")!=""){
						$data["value"]		= '<span class="category1-text">['.mbw_get_board_item("fn_category1").']</span> '.$data["value"];
					}
				}
				//댓글 개수 표시하기
				...
				$webzine_content			= strip_tags(mbw_htmlspecialchars_decode($webzine_content));
				$webzine_content			= trim(str_replace("&nbsp;", " ", $webzine_content));

				if(isset($data["maxlength"])){					
					$maxlength				= intval($data["maxlength"]);
				...
				if(mbw_get_vars("device_type")=="mobile"){
					...
					if(mbw_get_param("list_type")=="list"){
						...
						$template_start	.= '<a'.$title_url.' title="'.strip_tags($data["value"]).'"><div><span>'.$reply_space.$add_start_icon.$data["value"].$add_comment.$add_end_icon.'</span><br>';
		...
		if(!empty($data["description"])) $template_start	.= '<span class="description">'.$data["description"].'</span>';
...
if(!function_exists('mbw_get_view_template')){
	function mbw_get_view_template($data,$tag=null,$echo=true){
		...
		if(!empty($data["tpl"]) && $data["tpl"]!="item"){
			...
			if(!empty($data["link"])){
				$link_url			= $data["link"];
				$template_start	.= '<a href="'.$link_url.'" title="'.strip_tags($data["name"]).'">';
				$template_end	= '</a>'.$template_end;
			}
			...
			if(!empty($data["type"])){
				if($data["type"]=='title'){
					$template_start	.= '<span style="float:left;">'.$data["value"].'</span><span style="float:right;width:150px;text-align:right;">'.mbw_get_board_item('fn_reg_date').'</span>';
		...
		// 기본 태그 만들기 
			...
			if(!empty($data["type"])){			
				if($data["type"]=='content'){
					...
					if(mbw_get_board_option("fn_category_data")!=""){
						$template_start	.= mbw_get_category_template("SELECT",mbw_get_board_option("fn_category_data"));
					}
				}else{
			...
			if(!empty($data['description'])){
				if($echo) echo '<span class="description">'.$data['description'].'</span>';
				else $template_start	.= '<span class="description">'.$data['description'].'</span>';
			}
			if(!empty($data['add_end_html'])) $template_end		= $data['add_end_html'].$template_end;
			...
			if(!empty($data["type"])){
				...
				if($action=="list"){
					if($data["type"]=='cl_name_date'){
						$template_start	.= "<div><span".__STYLE($data["style"]).">'+reply_sign+value['".$mb_fields["select_comment"][$data["field"]]."']+'</span><span class=\"cmt-date\">'+value['".$mb_fields["select_comment"]["fn_reg_date"]."']+'</span></div>";
					}else if($data["type"]=='cl_content'){
						$template_start	.= "<div".__STYLE($data["style"]).">'+value['".$mb_fields["select_comment"][$data["field"]]."']+'</div>";
					}else{
						$template_start	.= "<div".__STYLE($data["style"]).">'+value['".$mb_fields["select_comment"][$data["field"]]."']+'</div>";
					}
				}else{
					if($data["type"]=='cw_name'){
						...
						$template_start	.= '<span  style="float:left";>'.mbw_get_user("fn_user_name").'</span><span style="float:right;"><label><input title="'.__MW("W_SECRET").'" type="checkbox" name="is_secret" value="1" '.$secret_checked.'/>'.__MW("W_SECRET").'</label></span>';
					}else if($data["type"]=='cw_content'){
						$template_start	.= '<textarea'.$data["ext"].__STYLE($data["style"]).' name="'.$data["item_name"].'" id="'.$data["item_id"].'">'.$data["value"].'</textarea>';
						$send_action				= "'".$action."'";
