<?php
	//반응형 클래스
	$responsive_class		= "col-543";		//col-432, col-555, col-444, col-333
	//갤러리 모델 설정
...
<div class="mb-style1 gallery-list">
			...
			if($list_data["total_count"] > 0){
				...
				if(!empty($board_items)){
					foreach($board_items as $item){
						...
						echo '<div class="gallery-item-box '.$category_item_class.' '.$responsive_class.'" style="">';
	...
	if(mbw_get_option("use_list_pagination")){
		if($list_data["total_count"] > 0){
			echo '<div id="'.$mb_board_name.'_pagination_box" class="pagination_box">'.mbw_get_pagination_template(array("total_count"=>$list_data["total_count"])).'</div>';
