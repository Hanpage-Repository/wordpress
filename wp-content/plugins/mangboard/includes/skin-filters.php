...
function mbw_filter_board_item($item,$data=null){
	...
	if($item["field"]=="fn_user_name"){	//USER NAME  FILTER
		...
		if(!empty($data["user_pid"])){
			$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"onclick"=>"getUserMenu('".$data["user_pid"]."','".$data[$mb_fields["board"]["fn_pid"]]."')","title"=>$item["value"]." ".substr($ip_address,0,-2)."**","class"=>"btn-user-info","type"=>"a"));
		}else{
			$item["value"]		= mbw_get_btn_template(array("name"=>$item["value"],"title"=>$item["value"]." ".substr($ip_address,0,-2)."**","class"=>"btn-user-info","type"=>"span"));
		}
