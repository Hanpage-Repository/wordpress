	...
	function mbw_get_admin_template($mode, $data){
		...
		if($item_type=='admin_board_name'){
			...
			if($board_type=="board" && intval(mbw_get_board_item("fn_list_level"))==0){
				$board_rss		.= '<a href="'.MBW_SITE_URL.'/?mb_trigger=rss&board_name='.mbw_get_board_item("fn_board_name2").'" target="_blank"> [RSS]</a>';
			}
			...
			if($item_type=='admin_select_skin_list'){
				$data["data"]		= implode(",",mbw_get_dir_entry("skins"));
			...
			if(mbw_get_board_item("fn_use_comment")=="1") $template_start	.= mbw_get_btn_template(array("name"=>__MW("W_COMMENT"),"href"=>admin_url('admin.php')."?page=mbw_board_options&board_name=".mbw_get_board_item("fn_board_name2")."&mode=comment","class"=>"btn btn-default margin-bottom-5"));
			$template_start	.= mbw_get_btn_template(array("name"=>"Copy","href"=>admin_url('admin.php')."?page=mbw_board_options&board_name=".mbw_get_param("board_name")."&category1=".mbw_get_param("category1")."&mode=write&board_action=write&board_pid=".mbw_get_board_item("fn_pid"),"class"=>"btn btn-default margin-bottom-5"));
			$template_start	.= mbw_get_btn_template(array("name"=>$data["name_btn"],"href"=>admin_url('admin.php')."?page=mbw_board_options&board_name=".mbw_get_param("board_name")."&category1=".mbw_get_param("category1")."&mode=write&board_action=modify&board_pid=".mbw_get_board_item("fn_pid"),"class"=>"btn btn-default"));
