	...
	function mbw_set_user_point($target,$action,$add_point=0,$user_pid="0",$user_name=""){
		...
		if(mbw_is_login() || !empty($user_pid)){		
			if($target=="comment"){
				...
				if($action=="write" || $action=="reply")	$sign			= "+";
				...
				if($action=="write")	$point		= intval(mbw_get_board_option("fn_point_board_write"));
					...
					$sign			= "-";
					$point		= intval(mbw_get_board_option("fn_point_board_write"));
				}
			}else if($target=="user"){
				$sign			= "+";
			...
			if(mbw_is_login() && $sign=="-" && $user_pid=="0"){
				$user_point		= intval(mbw_get_user("fn_user_point"));
				if($point>$user_point) $point		= $user_point;
			}
			...
			if($sign!="" && $point!=0){
				...
				if(!empty($user_pid)) $mdb->query($mdb->prepare("update ".$mb_admin_tables["users"]." set ".$mb_fields["users"]["fn_user_point"]."=".$mb_fields["users"]["fn_user_point"].$sign.$point." where ".$mb_fields["users"]["fn_pid"]."=%d",$user_pid));				
				//포인트 로그 남기기
...
if(!function_exists('mbw_get_level_item')){
	function mbw_get_level_item($level){
		...
		if(is_file(MBW_SKIN_PATH.$level_icon)) return ' <img class="user-i-level level-'.$level.'" src="'.MBW_SKIN_URL.$level_icon.'" />';
		else return "[".$level."]";
	}
}
...
if(!function_exists('mbw_logout')){
	function mbw_logout($mode="WP"){
		if(mbw_is_login()) mbw_set_log("logout","",array("mode"=>"user","board_action"=>"logout","board_name"=>"users"));
