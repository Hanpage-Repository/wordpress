...
if(mbw_get_param("mode")=="write" && mbw_get_param("board_action")=="modify"){
	...
	$mdb->query("update ".$mb_board_table_name." set ".$api_fields["fn_reply"]."=".$api_fields["fn_reply"]."+1 where ".$api_fields["fn_gid"]."='".$board_gid."' and 
	".$api_fields["fn_reply"].">".$board_reply);

}else if(mbw_get_param("board_action")=="delete"){
	log_trace(mbw_get_user("fn_user_level"));
...
if(!empty($query_command)){
	...
	if(mbw_get_param("mode")=="write"){
		...
		if(mbw_get_param("board_action")=="write" || mbw_get_param("board_action")=="reply" || mbw_get_param("board_action")=="modify"){
			...
			if(isset($api_fields["fn_image_path"])){			
				//대표 이미지 경로가 있을 경우
				if(!empty($send_data[$api_fields["fn_image_path"]])){
					...
					$image_path	= $mdb->get_var($mdb->prepare("select ".$mb_fields["files"]["fn_file_path"]." from `".$mb_admin_tables["files"]."` where ".$mb_fields["files"]["fn_board_pid"]."=%d and ".$mb_fields["files"]["fn_table_name"]."=%s and ".$mb_fields["files"]["fn_file_type"], $board_pid,$mb_board_table_name)." like 'image%' order by ".$mb_fields["files"]["fn_file_sequence"]." asc limit 1", ARRAY_A);
					if(!empty($image_path))
			...
			}else if(isset($api_fields["fn_user_picture"])){
				$image_path	= $mdb->get_var($mdb->prepare("select ".$mb_fields["files"]["fn_file_path"]." from `".$mb_admin_tables["files"]."` where ".$mb_fields["files"]["fn_board_pid"]."=%d and ".$mb_fields["files"]["fn_table_name"]."=%s and ".$mb_fields["files"]["fn_file_type"], $board_pid,$mb_board_table_name)." like 'image%' order by ".$mb_fields["files"]["fn_file_sequence"]." desc limit 1", ARRAY_A);
				if(!empty($image_path))
					$query_data[]		= $mdb->prepare("UPDATE ".$mb_board_table_name." set ".$api_fields["fn_user_picture"]."=%s where ".$api_fields["fn_pid"]."=%d", $image_path, $board_pid);
			}		
