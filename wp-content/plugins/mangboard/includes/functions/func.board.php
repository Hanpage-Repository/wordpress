...
if(!function_exists('mbw_get_image_url')){
	function mbw_get_image_url($type="",$data=""){
		$param		= "";		
		if($type=="download"){
			$param			= "?type=download&pid=".$data;
		}else if($type!=""){
			$param			= "?".$type."=".rawurlencode($data);
		}
		$image_url		= MBW_PLUGIN_URL."includes/mb-file.php".$param;
	...
	function mbw_set_params(){		
		if(mbw_get_trace("mbw_set_params")==""){
			...
			if(!$mb_check_set_param){
				...
				if(!empty($_GET["p"])){
					$mstore->set_params(mbw_get_decryption($_GET["p"]));
				}
				if(!empty($_GET)){
...
if(!function_exists('mbw_init_javascript')){
	function mbw_init_javascript(){
		...
		wp_localize_script( 'assets-js-common-js', 'ajax_object', array( 'ajax_url' => $admin_ajax_url,'admin_page' => (is_admin()? "true":"false")));

		global $mb_vars,$mb_api_urls,$mb_words;
		$script		= "";
		$script		.= '<script type="text/javascript">';
		$script		.= 'var mb_urls				= new Array();';
		$script		.= 'var mb_languages		= new Array();';
		$script		.= 'var mb_options			= new Array();';
		$script		.= 'var mb_categorys		= new Array("","","");';
		...
		$script		.= 'mb_urls["home"]					= "'.mbw_check_url(MBW_SITE_URL).'";';
...
if(!function_exists('mbw_get_move_script')){
	function mbw_get_move_script($type){
		global $mstore,$mb_languages;
		$move_script		= "";
		$site_url				= mbw_check_url(MBW_SITE_URL);
...
if(!function_exists('mbw_refresh_auth_cookie')){
	function mbw_refresh_auth_cookie(){
		...
		if($hmac==$hash){
			$expire		= mbw_get_timestamp() + 1209600;			
...
if(!function_exists('mbw_generate_access_token')){
	function mbw_generate_access_token(){		
		if(mbw_get_access_token()!=""){
			...
			mbw_set_cookie("mb_access_token",$auth_key);
			return $auth_key;
		}		
	}
}
		...
		if($mode=="today_visit"){	
			...
			if(mbw_get_cookie("mb_".$mode)==""){
				if(!empty($_SERVER['HTTP_REFERER'])){					
					...
					if(strpos(MBW_SITE_URL, $referer_host) === false){
