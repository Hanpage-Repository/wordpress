...
function mbw_dashboard_plugin_update($version){
	...
	$update_file		= download_url("https://downloads.wordpress.org/plugin/mangboard.".$version.".zip");
...
$rss_url					= "http://demo.mangboard.com/dashboard_rss.php?v=".$mb_version."&lang=".mbw_get_option("locale")."&site=".urlencode(MBW_SITE_URL);
...
if(!empty($rss)){
	...
	$dashboard_desc				= @html_entity_decode( $item[0]->get_description(), ENT_QUOTES, $dashboard_charset );
	...
	<div class="welcome-panel-content">
		...
		<div>
			...
			<div style="float:left;width:300px;">
				<div class="button"><a href="http://www.mangboard.com" target="_blank"><?php echo __MW('W_HOMEPAGE'); ?></a></div>
				<div class="button"><a href="http://www.mangboard.com/manual/" target="_blank"><?php echo __MW('W_MANUAL'); ?></a></div>
				<div class="button"><a href="http://www.mangboard.com/tech_support/" target="_blank"><?php echo __MW('W_TECH_SUPPORT'); ?></a></div>				
				<div class="button"><a href="http://cafe.naver.com/wpschool" target="_blank"><?php echo __MW('W_COMMUNITY'); ?></a></div>
			</div>
			<div class="clear"></div>
		</div>
		...
		<div class="welcome-panel-column-container">
			<div class="welcome-panel-column">
				...
				<div style="text-align:center;width:90%;">
					<?php
					$items		= $mdb->get_results("select * from `".$mb_admin_tables["analytics"]."` order by ".$mb_fields["analytics"]["fn_pid"]." desc limit 0,2",ARRAY_A);
					...
					echo '<tbody>';					
					echo '<tr><td>페이지 뷰</td><td>'.$items[1]["today_page_view"].'</td><td>'.$items[0]["today_page_view"].'</td></tr>';
					echo '<tr><td>게시물 수</td><td>'.(intval($items[1]["today_write"])+intval($items[1]["today_reply"])).'</td><td>'.(intval($items[0]["today_write"])+intval($items[0]["today_reply"])).'</td></tr>';
					echo '<tr><td>댓글 수</td><td>'.$items[1]["today_comment"].'</td><td>'.$items[0]["today_comment"].'</td></tr>';
					echo '<tr><td>신규 회원</td><td>'.$items[1]["today_join"].'</td><td>'.$items[0]["today_join"].'</td></tr>';
					echo '<tr><td>파일 업로드</td><td>'.$items[1]["today_upload"].'</td><td>'.$items[0]["today_upload"].'</td></tr>';
					echo '<tr><td>방문 횟수</td><td>'.$items[1]["today_visit"].'</td><td>'.$items[0]["today_visit"].'</td></tr>';
					echo '</tbody></table>';
			...
			<div class="welcome-panel-column welcome-panel-last">
				...
				<div style="text-align:center;width:90%;">
					...
					$url_items	= $mdb->get_results("select ".$field.", count(".$field.") from ".$mb_admin_tables["referers"]." where ".$mb_fields["referers"]["fn_reg_date"].">'".$search_date."' group by ".$field." order by count(".$field.") desc limit 6",ARRAY_A);
					echo '<table cellspacing="0" cellpadding="0" style="width:100%;">';
			...
			<div class="welcome-panel-column">
				...
				<div style="text-align:center;width:90%;">
					...
					echo '<tr><td>망보드 버젼</td><td>'.$mb_version.' '.$update_button.'</td></tr>';
					echo '<tr><td>PHP 버젼</td><td>'.phpversion()." (Upload Max: ".ini_get("upload_max_filesize").", Post: ".ini_get("post_max_size").')</td></tr>';
					echo '<tr><td>사용 언어</td><td>'.mbw_get_option("locale").'</td></tr>';					
					echo '<tr><td>전체 회원</td><td>'.$mdb->get_var("SELECT count(*) FROM ".$mb_admin_tables["users"]).'</td></tr>';
					echo '<tr><td>전체 파일</td><td>'.$mdb->get_var("SELECT count(*) FROM ".$mb_admin_tables["files"]).'</td></tr>';
					echo '<tr><td>전체 방문</td><td>'.$items[0]["total_visit"].'</td></tr>';					
					echo '</tbody></table>';
