...
if(!function_exists('mbw_validate_redirect')){
	function mbw_validate_redirect($redirect_to, $url=""){
		...
		else $site_url		= mbw_check_url(MBW_SITE_URL);
	...
	function mbw_get_ssl_url($url){
		...
		if(mbw_get_option("ssl_mode")){
			...
			if(mbw_get_option("ssl_domain")!=""){					
				$parse_url["host"]		= mbw_get_option("ssl_domain");
				if(strpos($parse_url["host"], 'http://') !== false)	$parse_url["host"]	= str_replace("http://", "", $parse_url["host"]);
			}
...
if(!function_exists('mbw_check_permalink')){
	function mbw_check_permalink($post_name,$site_url=""){		
		...
		if(empty($site_url)) $site_url		= mbw_check_url(MBW_SITE_URL);
...
if(!function_exists('mbw_get_decryption')){
	function mbw_get_decryption($data){
		$get_params		= array();
		$temp_values		= array();
		$param_data		= explode("&", base64_decode($data));
