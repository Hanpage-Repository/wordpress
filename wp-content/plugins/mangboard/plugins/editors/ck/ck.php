...
add_action('mbw_load_editor_'.$editor_type, 'mbw_load_editor_c',1); 
if(!function_exists('mbw_editor_ck_init')){
	function mbw_editor_ck_init(){
		if(mbw_get_vars("device_type")=="mobile"){
			wp_register_script('ck-editor-js', 'http://cdn.ckeditor.com/4.5.4/basic/ckeditor.js');	
		}else{
			wp_register_script('ck-editor-js', 'http://cdn.ckeditor.com/4.5.4/standard/ckeditor.js');	
		}	
...
if(!function_exists('mbw_editor_ck_template')){
	function mbw_editor_ck_template($action, $data){
		...
		$item_html		.= '<script>jQuery(document).ready(function(){ckeditor = CKEDITOR.replace( "'.$data["item_id"].'",{"customConfig": "'.MBW_PLUGIN_URL.'plugins/editors/ck/'.$config_name.'.js","filebrowserUploadUrl":"'.$admin_ajax_url.'?mode=basic&action=mb_uploader"} );});</script>';
		echo $item_html;	
	}
}
add_action('mbw_editor_'.$editor_type, 'mbw_editor_ck_template',1,2); 
