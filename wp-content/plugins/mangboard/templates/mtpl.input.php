	...
	function mbw_get_input_template($mode, $data){
		...
		if(strpos($item_type,'file')===0){
			if($item_type=='file_download'){			
				...
				if(!empty($file_data)){
					foreach($file_data as $file){
						...
						$template_start	.= '<a href="javascript:;"'.$data["ext"].__STYLE($data["style"]).' title="Download : '.$file[$mb_fields["files"]["fn_download_count"]].'" onclick="sendBoardFileData('.$file[$mb_fields["files"]["fn_pid"]].');return false;"><span >'.$file[$mb_fields["files"]["fn_file_name"]].' <span class="file-size">('.$file_size.')</span></span></a>';
					}
				}
			}else{
			...
			if(!empty($data["value"])){
				...
				$img_link		= "";
				$file_name		= "";
				if(!empty($data["size"])) $data["size"]		= "&size=".$data["size"];
				else $data["size"]		= "";
				...
				if(strpos($data["value"],'http')===0){
					...
					$img_link	= mbw_get_image_url("path",$data["value"]);
				...
				if(!empty($img_url)){
					...
					if($item_type=='img'){						
						...
						$template_start	= '<img'.$data["ext"].__STYLE("max-width:".$data["width"].";height:".$data["height"].";".$data["style"]).' src="'.$img_url.'"'.$file_attr.'/>';
						...
						$template_start	= '<a href="'.$img_link.'" target="_blank"><img'.$data["ext"].__STYLE("max-width:".$data["width"].";height:".$data["height"].";".$data["style"]).' src="'.$img_url.'"'.$file_attr.'/></a>';
			...
			if($item_type=='textarea'){
				...
				else if($item_type=='text_calendar') wp_enqueue_style('jquery-ui-css');
		...
		//html5
			...
			for($i=0;$i<$count;$i++){
				...
				if($t_data[$i]==$data["value"]){
					$template_start	.= '<option value="'.$t_data[$i].'" selected>'.$t_label[$i].'</option>';
					$item_check		= true;
				}else{
					$template_start	.= '<option value="'.$t_data[$i].'">'.$t_label[$i].'</option>';
				}
			}
