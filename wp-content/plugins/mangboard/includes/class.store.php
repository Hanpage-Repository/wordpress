...
Class MStore
{
	...
	public function set_board_options($board_name){		
		...
		if(empty($this->board_options)){
			...
			if(is_file(MBW_SKIN_PATH."includes/languages/skin-languages-".$this->get_option("locale").".php")){
				require_once(MBW_SKIN_PATH."includes/languages/skin-languages-".$this->get_option("locale").".php");
	...
	public function set_board_files($pid=0,$table_name=""){
		global $mb_admin_tables;
		global $mb_fields,$mb_board_table_name;
		if(!empty($pid) && !isset($this->board_files)) {			
			if(empty($table_name))		$table_name		= $mb_board_table_name;

			$this->board_files		= $this->db->get_results($this->db->prepare("select * from ".$mb_admin_tables["files"]." where `".$mb_fields["files"]["fn_table_name"]."`=%s and ".$mb_fields["files"]["fn_board_pid"]."=%d and ".$mb_fields["files"]["fn_is_download"]."='1' order by ".$mb_fields["files"]["fn_file_sequence"]." asc", $table_name,$pid),ARRAY_A);
		}
	}
	public function get_board_files($pid=0,$table_name=""){
		if(!empty($pid) && !isset($this->board_files)) $this->set_board_files($pid);
		
		if(!empty($this->board_files))
			return $this->board_files;
		else return "";
	}
	public function get_board_file($pid=0,$table_name=""){
		if(!empty($pid) && !isset($this->board_files)) $this->set_board_files($pid);
